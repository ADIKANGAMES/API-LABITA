...
class ventilation extends conexion{
    ...
    public function listaVentilation($pagina = 1){
        ...
        $query = "SELECT id,date_init,date_end,hour_init,hour_end,sensors FROM " . $this->table . " limit $inicio,$cantidad";
    ...
    public function insertarVentilation(){
        $query = "INSERT INTO " . $this->table . " (date_init,date_end,hour_init,hour_end,sensors) VALUES ('".$this->date_init."','".$this->date_end."','".$this->hour_init."','".$this->hour_end."','".$this->sensors."')";
        ...
        if(!isset($datos['token'])){
            ...
            if($arrayToken){
                if(!isset($datos['id'])){
                    ...
                    $this->id = $datos["id"];
                    if(isset($datos['date_init'])){ $this->date_init = $datos["date_init"]; }
                    if(isset($datos['date_end'])){ $this->date_end = $datos["date_end"]; }
                    if(isset($datos['hour_init'])){ $this->hour_init = $datos["hour_init"]; }
                    if(isset($datos['hour_end'])){ $this->hour_end = $datos["hour_end"]; }
                    if(isset($datos['sensors'])){ $this->sensors = $datos["sensors"]; }
